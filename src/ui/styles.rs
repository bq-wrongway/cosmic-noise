...
mod tests {
    ...
    fn test_border_creation() {
        let border = rounded_border(8.0);
        assert_eq!(border.width, 1.0);
        assert_eq!(border.color, Color::TRANSPARENT);

        let red_color = Color::from_rgb(1.0, 0.0, 0.0);
        let colored = colored_border(red_color, 2.0, 4.0);
        assert_eq!(colored.color, red_color);
        assert_eq!(colored.width, 2.0);
    }
    ...
    fn test_theme_colors() {
        ...
        let _bg = app_background_color(&theme);
        let _primary = primary_color(&theme);
        let _secondary = secondary_color(&theme);
        let _danger = danger_color(&theme);
        let _success = success_color(&theme);
        let _warning = warning_color(&theme);
    }
    ...
    fn test_text_styles() {
        ...
        let _error = error_text_style(&theme);
        let _success = success_text_style(&theme);
        let _warning = warning_text_style(&theme);
        let _primary = primary_text_style(&theme);
        let _secondary = secondary_text_style(&theme);
        let _muted = muted_text_style(&theme);
    }
    ...
    fn test_button_styles() {
        ...
        let _active = card_button_style(&theme, button::Status::Active);
        let _hovered = card_button_style(&theme, button::Status::Hovered);
        let _pressed = card_button_style(&theme, button::Status::Pressed);
        let _disabled = card_button_style(&theme, button::Status::Disabled);
    }

    #[test]
    fn test_slider_style() {
        let theme = Theme::TokyoNight;

        // Test slider style creation
        let _style = volume_slider_style(&theme, slider::Status::Active);
    }
}
